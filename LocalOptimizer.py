...
class CausalGNN (nn.Module):
    def __init__(self, n_species, d, d_prime, A=None, device=None):
        super().__init__()
        if A == None:
        ...
        self.norm = nn.LayerNorm((n_species, d_prime))
...
class CausalityInference (nn.Module):
    def __init__(self, n_species,
        ...
        self.gnn = CausalGNN(n_species, d, d_prime, device=device)
...
#                        CausalGNN(n_species, d_prime, d_prime, A=self.A))
        ...
        self.mlps = nn.ModuleList([
                        ResNetBlock(d, d_prime, d, dropout=0) for i in range(n_species)
                    ])
    ...
    def forward(self,
        ...
        out_causal, out_std = out_causal.squeeze(), out_std.squeeze()
